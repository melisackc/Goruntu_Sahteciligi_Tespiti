...
class SIFTDetector:
    # SIFT algoritması ile kopya-taşıma sahteciliği tespiti yapan sınıf.
    ...
    def __init__(self):
        try:
            ...
            raise RuntimeError(f"SIFT Sınıfı ilklendirilirken hata oluştu: {str(e)}")
    ...
    def analyze(self, image_path):
        try:
            ...
            if risk_score <= 30:
                ...
                message = "Görsel üzerinde SIFT analizi ile belirgin bir kopyala-yapıştır izine rastlanmamıştır. Doku sürekliliği korunmuştur."
                recommendation = "Görüntüde belirgin bir kopyala-yapıştır izi tespit edilmedi."
            elif 31 <= risk_score <= 65:
                risk_level = "Orta Risk"
                message = "Görsel içerisinde SIFT ile şüpheli bazı benzer alanlar ve tekrarlanan doku blokları tespit edilmiştir. Manipülasyon şüphesi mevcuttur."
                recommendation = "Görüntüde bazı benzer bölgeler tespit edildi. Görselin manuel olarak incelenmesi önerilir."
            else:
                risk_level = "Yüksek Risk"
                message = "Görselde SIFT ile çok sayıda birbirine tıpatıp uyan alan kopyalaması belirlenmiştir. Yüksek ihtimalle kopyala-yapıştır manipülasyonu yapılmıştır."
            ...
            raise RuntimeError(f"SIFT analizi gerçekleştirilirken bir hata oluştu: {str(e)}")
